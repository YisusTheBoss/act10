...
#+-------------------------------------------------------model 2
    def get_model(self):
        ...
        yr1 = layers.Conv1D(10, (3),padding='same', activation='sigmoid', input_shape=(self.look_back,1))(val)
        yr = layers.Conv1D(10, (5),padding='same', activation='sigmoid')(yr1)
        cr1 = layers.Conv1D(10, (7), padding='same', activation='sigmoid')(yr) 
        cr = layers.Conv1D(10, (9), padding='same', activation='sigmoid')(cr1)
        crs=layers.add([cr, yr1])
        crs1 = layers.Conv1D(10, (9),padding='same', activation='sigmoid')(crs)
        yr1 = layers.Conv1D(10, (5),padding='same', activation='sigmoid')(yr) 
        yr = layers.add([yr1, yr])                           
        yr2=layers.Conv1D(10, (7),padding='same', activation='sigmoid')(yr)
        
        ###################nueva capa
        yr3a=layers.Conv1D(10, (3),padding='same', activation='sigmoid')(yr2)
        yr3 = layers.add([yr3a, crs1])
        yr3b=layers.add([yr3a, crs1]) 
        yr4= layers.Conv1D(10, (3),padding='same', activation='sigmoid')(yr3b)
        ######################
   ...
   ###callback
        ...
        model.fit(t1,t2,self.look_back,self.num_epoch,verbose=1,validation_split=0.005, callbacks=[tb_callback])
        model.fit(t1,t2,self.look_back,self.num_epoch,verbose=1,validation_split=0.005)
        return model
